...
namespace ByteBank
{
    class Program
    {
        static void showMenu()
        {
            ...
            Console.WriteLine(" 4 - Configurações da conta");
        ...
        static int login(List<string> cpfs, List<string> senhas)
        {
            ...
            if (cpfs.Contains(cpfUsuario) && senhas.Contains(senhaUsuario))
                return cpfs.IndexOf(cpfUsuario);

            return -1;
        }

        static int CriarConta(List<string> cpfs, List<string> titulares, List<string> senhas, List<double> saldos, List<string> historico)
        {
            Console.Write("Informe o seu CPF: ");
            ...
            if (!cpfs.Contains(cpf))
            {
                ...
                Console.Write("Agora, informe o nome do titular: ");
                titulares.Add(Console.ReadLine());

                Console.WriteLine();

                Console.Write("Para finalizar, cadastre uma senha forte: ");
        ...
        static void DetalhesDoUsuario(List<string> cpfs, List<string> titulares, List<double> saldos, int indexCPFLogado)
        {
            Console.WriteLine();
            Console.WriteLine("--------------------------------");
        ...
        static void Manipular(List<string> cpfs, List<string> titulares, List<double> saldos, int indexCpfLogado, List<string> historico)
        {
            int option;
        ...
        {
            Console.Clear();
            Console.Write("Informe o nome atualizado: ");
            string novoNome = Console.ReadLine();

            titulares[cpfIndex] = novoNome;
            cadastrarTransacao($"Nome alterado para {novoNome}\n", historico, cpfIndex);
        ...
        static void DeletarUsuario(List<string> cpfs, List<string> titulares, List<string> senhas, List<double> saldos, int indexCpfLogado)
        {
        ...
        static void Main()
        {
            ...
            while (true)
            {
                ...
                while (true)
                {
                    ...
                    {
                        Console.Clear();
                        indexCpfLogado = CriarConta(cpfs, titulares, senhas, saldos, historico);
                ...
                do
                {
                    ...
                    switch (option)
                    {
                        case 0:
                            ...
                            break;
                        case 1:
                            DetalhesDoUsuario(cpfs, titulares, saldos, indexCpfLogado);
                            break;
                        case 2:
                            showBalance(saldos, indexCpfLogado);
                            break;
                        case 3:
                            Manipular(cpfs, titulares, saldos, indexCpfLogado, historico);
                            break;
                        case 4:
                            updateAccount(cpfs, titulares, senhas, indexCpfLogado, historico);
                            break;
                        case 5:
                            DeletarUsuario(cpfs, titulares, senhas, saldos, indexCpfLogado);
                            option = 0;
                            break;
                        case 6:
                            Console.Clear();
